...
function getFlavorByIndex(flavorsArray, flavorsIndex){
  return(flavorsArray[flavorsIndex]);
}

console.log(getFlavorByIndex(originalFlavors, 15));
...
function removeFlavorByName(flavorsArray, flavorToRemove){
  let indexNum = flavorsArray.indexOf(flavorToRemove);
  flavorsArray.splice(indexNum,1);
  return flavorsArray;
}
...
function filterByWord(flavorsArray, flavorSearch){
  const flavSearchArray = [];
  for(let i = 0; i < flavorsArray.length; i++) {
    if (flavorsArray[i].includes("Chocolate")) {
      flavSearchArray.push(flavorsArray[i]);
  }
  }
  return(flavSearchArray);
}
